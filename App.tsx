...
const App: React.FC = () => {
  ...
  // 修正：加入 event 參數來阻止冒泡，解決 iPad 雙重觸發
  ...
  // 修正倒數計時：用 useEffect 監聽 timeLeft 歸零
  useEffect(() => {
    let timer: NodeJS.Timeout;
    if (stage === GameStage.VICTORY) {
      setTimeLeft(40); // 確保每次進入都重置
      timer = setInterval(() => {
        setTimeLeft(prev => {
          if (prev <= 1) return 0;
          return prev - 1;
  ...
  return (
    // 👇 修正：h-[100dvh] 解決 iPad Safari 下方 Bar 遮擋問題
      ...
      {currentLevel && (
        <div className="w-full h-full relative overflow-hidden mario-transition" style={{ backgroundImage: getBackgroundImage(), backgroundSize: 'cover', backgroundPosition: 'center', backgroundColor: getBackgroundColor() }}>
          <div className="absolute inset-0 w-full h-full pointer-events-none z-0">
            {/* 👇 修正：刺蝟放大 (w-38%) 並上移 (bottom-28%)，避開對話框 */}
            {stage !== GameStage.LEVEL_4 && (<div className="absolute bottom-[28%] left-[2%] w-[38%] hero-float-animation"><img src={ASSETS.hedgehogBattle} alt="Hero" className="w-full object-contain drop-shadow-2xl" /></div>)}
            ...
            {stage === GameStage.LEVEL_4 && (
              <div className={`absolute inset-0 w-full h-full transition-opacity duration-1000 ${showReward ? 'opacity-0' : 'opacity-100'}`}>
                ...
                {/* Level 4 刺蝟位置同步修正 */}
                <div className="absolute bottom-[28%] left-[2%] w-[38%] z-20 hero-float-animation"><img src={ASSETS.hedgehogBattle} alt="Hero" className="w-full object-contain drop-shadow-2xl" /></div>
              </div>
            )}
          </div>
          {!showReward && (
            <div className="relative z-30 w-full h-full flex flex-col items-center justify-end pb-12">
              <div className="bg-white/95 backdrop-blur-md rounded-[2.5rem] p-8 md:p-12 shadow-[0_20px_60px_rgba(0,0,0,0.6)] border-[6px] border-blue-200 max-w-4xl w-[90%] md:w-[80%] animate-pop-in">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">{currentLevel.options.map((opt, idx) => (
                  // 👇 修正：事件改為 handleCorrect(e) 來傳遞事件物件
                  <button key={idx} onClick={(e) => opt.isCorrect ? handleCorrect(e) : handleWrong(e)} className="group relative bg-blue-50 hover:bg-yellow-50 border-4 border-blue-100 hover:border-yellow-400 p-6 rounded-[1.5rem] transition-all duration-200 text-left shadow-md hover:shadow-lg active:scale-95">
        ...
        <div className="relative w-full h-full text-center animate-pop-in p-6 z-30 bg-cover bg-center" style={{ backgroundImage: `url('${ASSETS.endBg}')` }}>
          ...
          <div className="absolute inset-x-0 bottom-0 p-4 pb-12 flex flex-col items-center justify-center gap-4 z-10">
            ...
            <div className="flex gap-4 md:gap-6 justify-center w-full">
              <button className="bg-blue-600 cursor-default text-white px-6 py-3 md:px-10 md:py-4 rounded-full text-xl md:text-2xl font-black shadow-lg opacity-90 whitespace-nowrap">前往購買</button>
              <button onClick={resetGame} className="bg-white text-gray-600 border-4 border-gray-200 px-6 py-3 md:px-10 md:py-4 rounded-full text-xl md:text-2xl font-black hover:bg-gray-50 transition-colors whitespace-nowrap">回到首頁</button>
            </div>
          </div>
        </div>
      )}
