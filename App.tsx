...
const App: React.FC = () => {
  ...
  const handleCorrect = () => {
    if (isProcessing) return; // 鎖定防止連點
    setIsProcessing(true); // 上鎖
  ...
  return (
    // 👇 [修正] 使用 h-[100dvh] 解決 iPad Safari 下方工具列遮擋問題
    <div className="relative w-screen h-[100dvh] overflow-hidden bg-[#060b28] select-none font-sans">
      ...
      {currentLevel && (
        <div className="w-full h-full relative overflow-hidden mario-transition" style={{ backgroundImage: getBackgroundImage(), backgroundSize: 'cover', backgroundPosition: 'center', backgroundColor: getBackgroundColor() }}>
          <div className="absolute inset-0 w-full h-full pointer-events-none z-0">
            {/* 👇 [修正] 刺蝟位置：iPad 上升 (bottom-25%)，電腦維持 (md:bottom-15%) */}
            {stage !== GameStage.LEVEL_4 && (<div className="absolute bottom-[25%] md:bottom-[15%] left-[5%] w-[35%] md:w-[30%] hero-float-animation"><img src={ASSETS.hedgehogBattle} alt="Hero" className="w-full object-contain drop-shadow-2xl" /></div>)}
            ...
            {stage === GameStage.LEVEL_4 && (
              <div className={`absolute inset-0 w-full h-full transition-opacity duration-1000 ${showReward ? 'opacity-0' : 'opacity-100'}`}>
                ...
                {/* 👇 [修正] Level 4 刺蝟位置：iPad 上升 */}
                <div className="absolute bottom-[25%] md:bottom-[15%] left-[5%] w-[35%] md:w-[30%] z-20 hero-float-animation"><img src={ASSETS.hedgehogBattle} alt="Hero" className="w-full object-contain drop-shadow-2xl" /></div>
              </div>
            )}
          </div>

          {/* 👇 [修正] 對話框位置與大小 */}
          {/* 使用 p-5 (iPad內距縮小), text-xl (iPad字體縮小) */}
          {!showReward && (
            <div className="relative z-30 w-full h-full flex flex-col items-center justify-end pb-8 md:pb-12">
              <div className="bg-white/95 backdrop-blur-md rounded-[2.5rem] p-5 md:p-12 shadow-[0_20px_60px_rgba(0,0,0,0.6)] border-[6px] border-blue-200 max-w-4xl w-[95%] md:w-[80%] animate-pop-in">
                ...
                <h2 className="text-xl md:text-4xl text-gray-800 font-black mb-6 md:mb-10 leading-snug">{currentLevel.question}</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">{currentLevel.options.map((opt, idx) => (<button key={idx} onClick={opt.isCorrect ? handleCorrect : handleWrong} disabled={isProcessing} className="group relative bg-blue-50 hover:bg-yellow-50 border-4 border-blue-100 hover:border-yellow-400 p-4 md:p-6 rounded-[1.5rem] transition-all duration-200 text-left shadow-md hover:shadow-lg active:scale-95"><div className="flex items-center gap-4"><div className="w-10 h-10 bg-blue-500 text-white rounded-full flex shrink-0 items-center justify-center font-black text-lg border-2 border-white shadow">{idx === 0 ? 'A' : 'B'}</div><span className="text-lg md:text-2xl font-black text-gray-700 group-hover:text-gray-900">{opt.text}</span></div></button>))}</div>
              </div>
            </div>
          )}
          {showReward && (
      ...
      {/* SUMMARY (解說頁) */}
      {stage === GameStage.SUMMARY && (<div className="w-full h-full relative flex flex-col items-center justify-center bg-cover bg-center animate-fade-in" style={{ backgroundImage: `url('${ASSETS.summaryBg}')` }}><div className="absolute inset-0 bg-white/40 backdrop-blur-[3px]"></div><div className="relative z-10 flex flex-col items-center max-w-5xl w-full p-4"><div className="relative mb-6"><div className="absolute inset-0 bg-yellow-400 blur-2xl opacity-50 rounded-full animate-pulse"></div>

        {/* 👇 [修正] 刺蝟圖片大小：iPad (w-60), 電腦 (md:w-96) */}
        <img src={ASSETS.hedgehogEnd} alt="Cici Sleeping" className="w-60 md:w-96 relative z-10 hero-float-animation" /></div>

        <div className="bg-white/95 rounded-[3rem] border-8 border-yellow-400 p-8 md:p-12 shadow-2xl text-center relative w-full"><h2 className="text-3xl md:text-5xl font-black text-blue-900 mb-6 font-['ZCOOL_KuaiLe']">天絲 Plus+ 的秘密</h2><p className="text-xl md:text-2xl text-gray-700 font-bold leading-relaxed mb-8 text-left md:text-center px-4">使用 Micro LF 級天絲纖維，透過特殊工藝處理，<br className="hidden md:block" />有效降低原纖化現象，即使多次洗滌也能<br className="hidden md:block" /><span className="text-yellow-600 font-black text-3xl">防止起毛球</span>，維持光澤與柔軟觸感！</p><button onClick={handleSummaryNext} className="w-full md:w-auto bg-gradient-to-r from-blue-500 to-blue-600 text-white px-16 py-5 rounded-full text-3xl font-black shadow-lg hover:scale-105 transition-transform active:scale-95">下一頁 ➔</button></div></div></div>)}
      ...
      {stage === GameStage.VICTORY && (
        <div className="relative w-full h-full text-center animate-pop-in p-6 z-30 bg-cover bg-[center_top]" style={{ backgroundImage: `url('${ASSETS.endBg}')` }}>
          ...
          {/* 內容容器：置於底部 */}
          <div className="absolute inset-x-0 bottom-0 p-4 pb-8 md:pb-12 flex flex-col items-center justify-center gap-4 z-10">

            {/* 👇 [修正] 優惠框寬度：iPad (w-90%), 電腦 (md:w-[600px]), 高度自動 (h-auto) */}
            <div className="bg-white/95 rounded-[2rem] border-8 border-yellow-400 shadow-2xl mb-2 
                            w-[90%] md:w-[600px] h-auto p-6 md:p-10
                            flex flex-col justify-center items-center mx-auto relative">
              <p className="text-xl md:text-3xl font-black text-gray-800 mb-6 leading-normal">
                請拍攝此畫面，購買 <span className="text-blue-600">"天絲PLUS雲柔被1件"</span>，結帳時出示此畫面
              </p>
              <p className="text-red-500 text-2xl md:text-4xl font-black mb-4 animate-pulse">
                加贈 "限量版小童枕1個"
              </p>
              <p className="text-gray-500 font-bold text-lg">(限時優惠，請把握機會！)</p>
            </div>
